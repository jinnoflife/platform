...
class ThemeService
{
    ...
    public function getThemeConfiguration(string $themeId, bool $translate, Context $context): array
    {
        ...
        $supportTexts = array_replace_recursive($baseTheme->getSupportTexts() ?? [], $theme->getSupportTexts() ?? [], $translations);
        if ($translate && !empty($supportTexts)) {
            $configFields = $this->translateSupportTexts($configFields, $supportTexts);
        }
